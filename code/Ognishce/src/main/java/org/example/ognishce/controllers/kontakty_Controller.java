...
public class kontakty_Controller {
    ...
    @PostMapping("/kontakty_authenticate")
    public String authenticateUser(String login, String password, HttpSession session, Model model) {
        ...
        if (user != null) {
            ...
            if (user.getPassword().equals(password)) {
                ...
                return "redirect:/POLZOVATEL_kontakty";
            } else {
    ...
    @PostMapping("/kontakty_registration")
    ...
    @PostMapping("/kontakty_authenticate_admin")
    public String login(@RequestParam String login, @RequestParam String password, Model model) {
        if ("admin".equals(login) && "1111".equals(password)) {
            return "redirect:/ADMIN_kontakty";
        } else {
            model.addAttribute("kontakty", true);
            return "kontakty";
        }
    }
}
